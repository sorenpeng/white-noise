...
const AudioVisualizer: React.FC<AudioVisualizerProps> = ({
  ...
  const drawSpectrum = (ctx: CanvasRenderingContext2D, width: number, height: number, frequencyData: Uint8Array) => {
  ...
  const drawWaveform = (ctx: CanvasRenderingContext2D, width: number, height: number, waveformData: Uint8Array) => {
  ...
  const drawParameterIndicators = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    <div className={styles.visualizer}>
      ...
      <div className={styles.canvasContainer}>
        ...
        <div className={styles.indicators}>
          <div className={styles.indicator}>
            <div className={styles.indicatorDot} style={{ backgroundColor: `hsla(0, ${60 + energy * 40}%, ${50 + energy * 30}%, ${0.6 + energy * 0.4})` }} />
            <span className={styles.indicatorLabel}>能量</span>
          </div>
          <div className={styles.indicator}>
            <div className={styles.indicatorDot} style={{ backgroundColor: `hsla(60, 70%, ${30 + brightness * 50}%, ${0.6 + brightness * 0.4})` }} />
            <span className={styles.indicatorLabel}>明暗</span>
          </div>
          <div className={styles.indicator}>
            <div className={styles.indicatorDot} style={{ backgroundColor: `hsla(120, ${60 + speed * 40}%, ${50 + speed * 20}%, ${0.6 + speed * 0.4})` }} />
      ...
      <div className={styles.stats}>
        <div className={styles.stat}>
          ...
          <span className={styles.statValue}>{visualMode === 'spectrum' ? '频谱分析' : '波形显示'}</span>
        </div>
        <div className={styles.stat}>
